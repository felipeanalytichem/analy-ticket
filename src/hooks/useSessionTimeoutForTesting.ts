...
export function useSessionTimeoutForTesting(config: SessionTimeoutConfig = {}) {
  ...
  // TESTING FUNCTIONS - Force timeout by setting old activity time
  const forceTimeout = useCallback(() => {
    if (!isActiveRef.current) {
      console.log('❌ Cannot force timeout - monitoring not active');
      return;
    }
    
    console.log('🧪 Forcing session timeout for testing...');
    
    // Set last activity to a very old time (older than timeout)
    const veryOldTime = new Date(Date.now() - (configRef.current.timeoutMinutes + 1) * 60 * 1000);
    lastActivityRef.current = veryOldTime;
    
    console.log(`🕰️ Set fake activity time to: ${veryOldTime.toLocaleTimeString()}`);
    
    // Trigger immediate check
    checkSessionStatus();
  }, [checkSessionStatus]);

  // Force warning for testing
  const forceWarning = useCallback(() => {
    if (!isActiveRef.current) {
      console.log('❌ Cannot force warning - monitoring not active');
      return;
    }
    
    console.log('🧪 Forcing session warning for testing...');
    
    // Set last activity to time that will trigger warning
    const warningTime = new Date(Date.now() - (configRef.current.timeoutMinutes - configRef.current.warningMinutes + 0.1) * 60 * 1000);
    lastActivityRef.current = warningTime;
    warningShownRef.current = false; // Reset warning flag
    
    console.log(`⚠️ Set activity time to trigger warning: ${warningTime.toLocaleTimeString()}`);
  ...
  return {
    ...
    startMonitoring,
    stopMonitoring,
    forceTimeout, // Testing function
    forceWarning, // Testing function
    config: finalConfig,
  };
} 
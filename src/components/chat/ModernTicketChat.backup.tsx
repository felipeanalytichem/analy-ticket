...
export function ModernTicketChat({ ticketId, className }: ModernTicketChatProps) {
  const { user } = useAuth();
  ...
  // Get remaining time for deletion (in seconds)
  const getRemainingDeleteTime = (message: MessageWithReactions): number => {
    const messageTime = new Date(message.created_at).getTime();
    const currentTime = new Date().getTime();
    const timeDifference = currentTime - messageTime;
    const fiveMinutesInMs = 5 * 60 * 1000; // 5 minutes
    const remainingMs = fiveMinutesInMs - timeDifference;
    return Math.max(0, Math.floor(remainingMs / 1000));
  };

  // Check if message can be deleted (within 5 minutes and by sender)
  const canDeleteMessage = (message: MessageWithReactions): boolean => {
    if (!user || message.sender_id !== user.id) {
      console.log('🗑️ Cannot delete - not owner:', { userId: user?.id, senderId: message.sender_id });
      return false;
    }
    ...
    const canDelete = timeDifference <= fiveMinutesInMs && message.content !== '[Message deleted]';
    console.log('🗑️ Can delete message:', { 
  ...
  const handleDeleteMessage = async (messageId: string) => {
    ...
    try {
      console.log('🗑️ Calling deleteMessage service...');
  ...
  return (
    <div className={cn("flex flex-col h-[600px] bg-white dark:bg-gray-900 rounded-xl border dark:border-gray-700 shadow-sm overflow-hidden", className)}>
      ...
      <div className="bg-red-100 p-2 border-b">
        <Button
          onClick={() => {
            ...
            if (messages.length > 0) {
              ...
              console.log('🧪 Testing delete on last message:', lastMessage.id);
              handleDeleteMessage(lastMessage.id);
            } else {
                      ...
                      <div className="group relative">
                        ...
                        >
                          <div className="text-sm leading-relaxed">
                            {message.content}
                            {/* TEST DELETE BUTTON - Always visible for debugging */}
                            {isOwn && (
                              <Button
                                size="sm"
                                variant="destructive"
                                ...
                                onClick={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
                                  console.log('🗑️ TEST Delete button clicked!', message.id);
                                  alert(`TEST Delete clicked for message: ${message.id}`);
                          ...
                          <div className={cn(
                            "absolute top-2 opacity-100 transition-all duration-200 z-20",
                            isOwn ? "-left-12" : "-right-12"
                          )}>
                            <div className="flex items-center bg-white dark:bg-gray-800 rounded-full shadow-lg border border-gray-200 dark:border-gray-600 p-1">
                              <TooltipProvider>
                                <Tooltip>
                              ...
                              {isOwn && (
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  className="h-8 w-8 p-0 hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors"
                                  onClick={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    console.log('🗑️ Delete button clicked!', message.id);
                                    console.log('🗑️ Can delete:', canDeleteMessage(message));
                                    alert(`Delete clicked for message: ${message.id}`);
                                    if (canDeleteMessage(message)) {
                                      handleDeleteMessage(message.id);
                                    } else {
                                      alert('Cannot delete this message (too old or not yours)');
                                    }
                                  }}
                                >

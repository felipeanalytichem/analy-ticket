-- First, ensure the ticket_status type exists
DO $$ 
BEGIN
...
-- Create a function that accepts text parameters
...
-- Create an overloaded function that accepts ticket_status parameters
CREATE OR REPLACE FUNCTION public.log_ticket_activity(
  p_ticket_id    uuid,
  p_user_id      uuid,
  p_action_type  text,
  p_field_name   text,
  p_old_value    ticket_status,
  p_new_value    ticket_status,
  p_description  text DEFAULT NULL,
  p_metadata     jsonb DEFAULT '{}'::jsonb
)
RETURNS void
...
GRANT EXECUTE ON FUNCTION public.log_ticket_activity(uuid, uuid, text, text, ticket_status, ticket_status, text, jsonb) TO authenticated;

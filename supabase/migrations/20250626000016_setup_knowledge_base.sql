...
-- Create trigger for updating article stats
...
-- Create trigger to automatically generate slug for categories
...
-- Create trigger to automatically generate slug for articles
...
BEGIN
    ...
    NEW.reading_time_minutes := GREATEST(1, CEIL(array_length(regexp_split_to_array(NEW.content, '\s+'), 1)::float / 200));
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

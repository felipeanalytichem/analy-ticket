...
-- Recreate with proper type handling for ticket status
...
BEGIN
    ...
    -- Ensure status values are properly handled
    IF p_field_name = 'status' THEN
        -- Cast status values to text if they're not null
        p_old_value := COALESCE(p_old_value::TEXT, 'N/A');
        p_new_value := COALESCE(p_new_value::TEXT, 'N/A');
    END IF;

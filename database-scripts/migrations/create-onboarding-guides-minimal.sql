-- Create Onboarding Guides - Minimal Safe Version
-- This script uses only the core columns that definitely exist

-- First, drop any existing conflicting functions
DROP FUNCTION IF EXISTS create_onboarding_articles();
DROP FUNCTION IF EXISTS create_basic_onboarding_articles();

-- Create knowledge base categories first
...
-- Create a safe function using only core columns
CREATE OR REPLACE FUNCTION create_safe_onboarding_articles()
...
BEGIN
  ...
  -- Insert articles using only core columns that definitely exist
  ...
  RAISE NOTICE 'Successfully created 3 onboarding guides using core columns only!';

END;
$BODY$ LANGUAGE plpgsql;

-- Execute the function
SELECT create_safe_onboarding_articles();

-- Clean up
DROP FUNCTION create_safe_onboarding_articles(); 